//creating shopping cart using module
const shoppingCart = (function(){
    let basket=[];

    return {
        upsertItem : function(item){
            const index = basket.findIndex(eachItem => eachItem.id === item.id) //we check if there is an item by id ..
            if(index > -1){ // if it's greaterthan -1 which means it is there then we update
                basket[index]=item;
            }
                else{
                    basket.push(item); // if not we add the item inside the basket
                }
